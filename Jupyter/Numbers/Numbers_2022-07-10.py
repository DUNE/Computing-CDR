...
wallactual = {2021:{"Total":41.2,"Analysis":41.2-17.2-9.3,"MARS":17.2,"Production":9.3}}
...
configfile = "Parameters_2022-07-10-2040.json"
if len(sys.argv) > 1:
...
for dtype in Data.keys():
  det = "ProtoDUNE"
  #print (dtype)
  if "SP2" in Data[dtype]:
    Data[dtype][det] = Data[dtype]["SP"] + Data[dtype]["DP"] + Data[dtype]["SP2"] + Data[dtype]["PDVD"]
...
for det in CombinedDetectors:
  ...
  print ("Total-CPU",det,Data["Total-CPU"][det] )
...
for y in wallactual:
    coreactual[y] = {}
    cpuactual[y] = {}
    for c in wallactual[y]:
        coreactual[y][c] = wallactual[y][c]*MHrsPerYear/scaleTo2020
        cpuactual[y][c] = wallactual[y][c]*efficiency
...
s =  " ,\t CPU ,\tWall,\tWall F+MARS/Collab,\t\qquad  ,\t Tape\qquad,\t Tape\qquad  ,\t Disk\qquad  ,\t Disk\qquad \n"
s +=  "Years,\t(Mhrs),\t(Wall-Mhrs),\t(Wall-Mhrs),\tcores,\t Total(PB),\tF/C/Collab ,\t Total(PB) ,\tF/C/Collab\n"
...
for i in range(MinYears,len(Years)):  
    format = "%d,\t%4.0f,\t%4.0f,\t(%4.0f+%4.0f)/%4.0f,\t %5.0f,\t %8.1f,\t %5.1f/%5.1f/%5.1f,\t %8.1f,\t %5.1f/%5.1f/%5.1f s\n"
...
s = ""
i = -1
for thing in [diskpoints,tapepoints,cpuactual,coreactual,diskactual,tapeactual]:
