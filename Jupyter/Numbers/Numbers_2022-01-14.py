...
def cumulateold(a,lifetime=100):
  ...
  for i in range(0,len(a)):
    ...
    if i > 0 and begin > 0 and lifetimeextra > 0:
        b[i-1] += a[begin-1]*lifetimeextra
  if lifetimeextra != 0:
    print ("cumulate",a,b)
  return b
...
configfile = "Parameters_2022-01-16-2040.json"
  ...
  for key in DetectorParameters:
    ...
    if not "Sim" in key:
      ...
      if key in ["CPU","Reco"]:  # if doing reco, do over previous events using memory
            ...
            test = cumulateold(Inputs[det]["Events"],RecoMemory[det])*config[det][key]
      else:
...
# and same to make an FD from HD and VD
for dtype in Data.keys():
  det = "ProtoDUNE"
  #print (dtype)
...
for k in StorageTypes:
  ...
  test = cumulateold(Data[k]["Tape"],TapeLifetimes[k])
