...
\chapter{Data and Processing Volume Estimates \hideme{3/8 HMS numbers updated}}
...
As a starting point to understand the resources needed for DUNE Computing and development needed to utilize those resources, we start with an estimate of the data volumes and CPU needs from bottoms-up estimates. 
...
These assumptions have been coded into a python-based model and are updated frequently based upon changes to the \dwords{fd}, \dwords{nd} designs, and physics requirements.
...
DUNE's detectors will produce information from a variety of technologies.  We anticipate that raw data volumes will be dominated by the digitized waveforms from \dword{lar} detectors, and to a lesser extent \dwords{pd}. \dword{lar} TPCs read out over long time windows while the \dword{pd} detectors read out only above threshold, and we find that the \dword{lar} information dominates at the raw data level and drives the overall data volumes. % in the prototype runs at the \dword{cern}, the \dword{fd}, and the \dword{nd}.  
...
\dword{sp} and \dword{dp} prototype detectors that ran at the \dword{cern} in 2018 (\dword{sp}) and 2019(\dword{dp}). 
...
For example, the uncompressed \dword{sp} raw data for a single \dword{pdsp} trigger record were observed to be around 178\,MB in size, which is the amount expected for the number  of \dword{tpc} channels read + a 20\% overhead for other detectors and headers.  Compressed \dword{sp} raw data averages 75\,MB, consistent with compression by a factor of 2.5.  

%anne Dual phase data 
\subsection{ProtoDUNE Dual Phase Experience}
\dword{pddp} recorded signals using two \dword{crp} units during the 2019 run.  Observed data size without compression  was 110\,MB.  %Numbers for 2018 and 2019 have been 
...
The data volumes estimates presented here are based upon the current (Winter 2022) \dword{fd}  and \dword{nd} designs. This assumes that the first module will be \dword{hd}, the second module \dword{vd}, and a third \dword{hd} module added in early 2030s. The \dword{nd} data volume is based upon the final \dword{nd} design and ignores the phased approach that is currently planned in order to be conservation in our estimates. A separate discussion is provided for each \dword{fd} module type and the \dword{nd}. The summary plots and table at the end include data and simulation for all detectors.
...
Overall, bottoms-up estimates yield data volumes of around 9.4\,PB/year and 16\,PB/year for each \dword{hd} and \dword{vd} module respectively.  Lossless compression and restriction of the readout to geographical regions of interest should reduce this volume substantially. However, additional modules will  increase these rates.  A maximum rate of 30\,PB/year across all modules and modes of operation has been specified and it is assumed that DAQ and detector configurations will change to meet this specification.  We will note that 30\,PB/year is  an average of 1.3\,GB/sec, less than the rates already demonstrated for \dword{protodune} %acquisition 
