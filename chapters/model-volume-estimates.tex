...
\item lossless zero suppression, \todo{trj: does this mean to say lossless compression?  ZS sounds lossy}
...
For example, the uncompressed \dword{sp} data for a single \dword{pdsp} trigger record were observed to be around 178\,MB in size, which is the amount expected for the number  of \dword{tpc} channels read + a 20\% overhead for other detectors and headers.  Compressed \dword{sp} data averages 75\,MB, consistent with compression by a factor of 2.5.  
