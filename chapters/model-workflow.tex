...
these constraints, we have designed and prototyped a generalization of the \dword{sam} model: generic jobs arrive at grid worker nodes and ask a central Workflow Allocator what they are to work on and which file to process within that activity. This matching is based on the physical characteristics of the worker node job slot (such as memory, processors, and lifetime) and what data are unprocessed and suitable to be accessed from that execution site. 
