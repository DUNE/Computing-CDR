...
are expected to occur one to two times/month and involve data volumes of up to 160\,TB/module of uncompressed \dword{fd} data. The ability to produce a reasonably fast pointing signal would be extremely valuable to optical astronomers doing follow-up, especially if the supernova were in a region where dust masks the primary optical signal. The need to be alert to %supernovae 
...
Given the nominal rate of 7.98\,Gb/s from the \dword{daq}, and assuming that the network interface card (NIC)
...
The \dword{lan} for the Far and Near Sites will be designed, installed, and commissioned as a collaboration between the \dword{dune} \dword{daq} %Group 
...
Connection to European sites is accomplished via the \dword{esnet},
...
\dword{layer3} \dword{vrf} provision is now very prevalent in \dword{hep}. 
