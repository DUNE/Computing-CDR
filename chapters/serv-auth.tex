...
\dword{fnal} interactive login machines and \fixme{to determine?} who can obtain Grid X.509 credentials.
 ...
 During the time when the \dword{protodune} experiments NP02 and NP04 are running at the \dword{cern} %CERN Neutrino Platform, DUNE 
 collaborators %who are members of 
 working on those activities may also apply %to CERN 
 for interactive computer access at \dword{cern}.
...
\dword{osg} in the US and \dword{wlhc} % the Worldwide LHC 
...
The token issuer is populated with information from the FERRY server. % mentioned above. 
...
\dword{fnal} SSO (but allowing more Identity Providers)
for the foreseeable future.  It is likely that \dword{jwt} tokens will be used to access various non-interactive web
...
To this end DUNE Computing Management has expressed to \dword{fnal} the following requirements:
...
\dword{fnal} is responding, and as of this writing, users with \dword{cern} credentials can now use them to access the DUNE DocDB. \dword{fnal} is in the process of federating its SharePoint instances with \dword{cern}, and the DUNE wiki and Indico sites are next in line.
