...
DUNE relies on the host lab, \dword{fnal}, to provide a wide variety of services.  These include web, database, computing, and storage services as well as cyber security.  The networking services are described in \ref{ch:netw}.  We summarize the other services briefly.

\subsection{Web Services}
The conference-scheduling service (currently Indico), Sharepoint, the DUNE Document Database, and the DUNE Wiki, as well as the main {\tt dunescience.org} web page, are all hosted at \dword{fnal}.  %anne There are also 
...
The lab provides an electronic logbook service and %.  It also 
...
disk caching system is the front end to this system, although also containing some standalone disk.  The lab also provides a number of data management and data movement services, including Fermi-File Transfer Service, \dword{sam}, \dword{rucio}, \dword{metacat}, and the Data Dispatcher. 
...
Section~\ref{sec:ccb}. DUNE also receives a few services that are hosted at the \dword{cern}.  These include the EDMS document
management system and the MONIT monitoring system. Some DUNE meetings are organized using \dword{cern}'s Indico instance.  \dword{cern} also hosts the DUNE CRIC, which is the master list of all DUNE compute and storage resources as well as the means to track allocations. The ETF testing service, hosted and operated at \dword{cern}, routinely tests all remote compute sites on DUNE's behalf. %anne is also hosted and operated at \dword{cern}.
