...
Monitoring of DUNE jobs (WMS monitoring) is covered elsewhere. \fixme{(anne) find and reference} As the workflows are broadly similar to current \dword{lhc} experiment workflows, DUNE plans to reuse the relevant monitoring tools (\dword{etf}, \dword{perfsonar}) developed for this purpose by the \dword{wlcg}. %anne, which we cover in this section.
...
\dword{etf} is the \dword{wlcg} % WLCG is the ETF system.
 Experiments Tests Framework\cite{bib:ETFDoc},\cite{bib:ETFStatus}. It  has been known by different names, but the underlying framework remains the same. \dword{cern} plans to support this framework for as long as \dword{lhc} experiments require availability and reliability metrics from an independent source.
 
\dword{etf} is a system of tools that regularly test all the available resources for different experiments (\dwords{vo}). It has been developed and run on behalf of the \dword{wlcg} %anne experiments (VOs) 
...
Customized tests are in place to verify that a minimal simulation works on the worker nodes and to check that a given worker node has \dword{ipv6} networking. A customization to perform a lightweight test of access to  \dword{rucio} servers from the worker node is in progress.
...
We estimate that this effort will require for one FTE staff. %will be needed.

It is expected that \fixme{X} virtual machines with minimum spec \fixme{X} will be required to run the service, \fixme{to run ETF?} which can be located at any site (e.g., \dword{cern}).

It is expected another three FTE will be required to monitor the \dword{etf} results, probably as part of more general computing shifts. This will involve looking at the test results, following up on failing tests and opening tickets for failing resources. Site administrators can also use \dword{etf} tests to understand the performance of their sites. %anne: seems like it says the same thing -- and it can be used (as it is for the LHC VOs) to provide a measure of site performance (availability, reliability).

See Table~\ref{} \fixme{X} for a summary... 
...
each site that provides resources. The data from these \dword{perfsonar} boxes %is 
...
\end{dunetable}
\fixme{(anne) keep the above table? fill it in?}
