...
and file delivery.  Those functions are being separated with well defined interfaces.  The goal is to have the new systems in place before beam running begins on \dword{pdsp2} %HMS - is this ok now? anne ProtoDUNE-II (single-phase 
(horizontal drift).  
...
% Collections could be long trigger records, data from different detectors over a particular time period, …
 ...
 The replacement for \dword{sam} should support use of run or trigger record level information, in additional to file information,  as appropriate. 
...
\item{\item Data Dispatcher} The existing \dword{sam} system supports a station and \dword{samproject} ecosystem where \dwords{samdataset} are submitted for processing as \dwords{samproject}.  The \dword{samproject} is a server process that contains a list of files based on a \dword{samdataset} description  and, when asked, delivers location information for the next file in the list.  Request, delivery and processing are tracked and recorded. This allows resubmission on failure. A website allows users to see the details of file delivery. 
...
in the following subsections we describe the existing features in more detail with the replacement technologies in following sections. 
...
In the following example file metadata from DUNE, generic \dword{sam} values are listed first followed by experiment specific parameters. The items starting with beam.momentum and containing ``.'' in their names are parameters. Many of the parameter entries are specific to the run configuration, not the individual file and will move in the new design. 
...
We propose to replace the \dword{sam} system with a set of components. 

\begin{itemize}
\item A Metadata Catalog  which stores file characteristics but not locations.    
    \item A Data Ingest Manager which takes in data from detector sites, declares it to the catalog and transfers to the managed storage system.
    \item A Rucio-based Replica Manager which stores the physical location of files and provides tools to move them between storage elements. 
    \item A Data Dispatcher which interacts with user and production processing systems.  It provides file location information and can track file processing. 
\end{itemize}
...
run at the detector location, at CERN EHN1, SURF surface and Fermilab. 
...
Work has begun on a prototype \dword{datadis} which re-uses existing \dword{sam} code where appropriate.  The exact boundary between the \dword{datadis} and the Workload Manager (see Chapter \ref{ch:wkflow}) has not been finalized but it is likely that other \dword{fnal} experiments will need the \dword{datadis} functionality if \dword{sam} is deprecated. 
