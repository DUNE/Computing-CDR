...
\section{Requirements in replacing SAM functionality \hideme{Schellman/Mandrichencko - draft}}
...
files will be transferred to a dropbox on the nearest managed storage element, using FTS3 \todo{add a note about FTS3 to the glossary} as the transport mechanism.  
...
are moved from point to point as needed.  The choice that we have made for this functionality is the \dword{rucio} data
...
taken in the fall 2018 \dword{pdsp} (NP04) run and since that time, as well as all raw data taken by the \dword{pddp} (NP02) run, is tracked by Rucio.  All output of Monte Carlo simulation and reconstruction also is tracked by Rucio. 
...
data in the legacy \dword{sam} system into Rucio as well.  Currently about 50\% of the data by volume is known by Rucio but only about 25\% of the files by file count are declared to Rucio at the moment. 
...
Rucio has a internal metadata functionality but our evaluation indicates that it doesn't provide the full functionality currently provided by the \dword{sam} catalog, and thus DUNE has proceeded to develop our own metadata catalog. 
...
Although the main target user of the project is ProtoDUNE/DUNE, \dword{metacat} is supposed to be generic enough to be able to be used by multiple experiments.   \dword{metacat} has to satisfy the following general requirements: 
...
In \dword{sam} files were identified by a unique immutable name within a single namespace.  To support  multiple use cases (for example user and production spaces) \dword{metacat}s objects and be identified  by names within namespaces. The name of an object is unique within its namespace. A namespace/name pair uniquely identifies  an object in \dword{metacat}. In addition to namespace/name, files are assigned unique text identifiers. These identifiers are primarily for internal use within the \dword{metacat} database, but are available to the user. At the time of the file declaration, the user can specify a file id, which must be unique, otherwise \dword{metacat} will generate a unique file id. After a file is declared to \dword{metacat}, it can be renamed. Both namespace and name can be changed, providing the new namespace/name pair is unique. However, file id can not be changed. 
...
Here is an example with the same file describe in \dword{sam} above. Note the 9 fixed fields at the top followed by freer form metadata. 
...
A \dword{metacat} user in the system is identified  by a unique username. A user can be a member of zero or more roles (groups). Namespaces and attribute categories have owners. Namespace or category owner can be either an individual user or a role. If the namespace or the category is owned by the role, it means it is automatically owned by all role members. 
...
\subsection{Queries} 
One of the most important parts of the \dword{metacat} functionality is the ability to query the database for "interesting" files. Essentially, the query is a logical expression in terms of file and/or dataset metadata attributes, file provenance relationship and dataset parent/child relationship. There are two types of queries in \dword{metacat} -file queries and dataset queries. file queries return a set of files (a list of file ids) whereas dataset queries return list of datasets. The file and dataset lists are not guaranteed to have consistent order, so they are in fact sets rather than lists. 
A query is merely a formula specifying the selection criteria. \dword{metacat} never saves the results of the query, so re-running a query can produce different results as the files are added/removed from the system, to/from the datasets or their metadata attributes change. However, there is an option to save results of the file query as a new dataset or add selected files to an existing dataset. 
...
The original \dword{sam} query language was produced in the late 1990's to provide a restricted command set and  avoid having users execute full SQL queries.
