...
and file delivery.  Those functions are being separated with well defined interfaces.  The goal is to have the new systems in place before beam running begins on \dword{pdhd}.
...
and this additional file is reconstructed with v10, the \dword{samdataset} will grow. There are also \dword{samdataset} ``\dwords{samsnapshot}'' that are derived from \dwords{samdataset} and capture the exact files in the \dword{samdataset} when the \dword{samsnapshot} was made.
...
A prototype version of \dword{metacat} has been produced and described in reference~\cite{Mandrichenko:2021spd}. 
...
\item
It needs to scale to at least 100 million objects and likely into the 10s of billion objects. %Kirby - rough guess based on NOvA and MicroBooNE experience and 20 years of DUNE operations \todo{(anne) 100M OR beyond? How far beyond? This is ambiguous}
