...
\begin{document}
\chapter{Resource Needs Summary\hideme{coll-org.tex Anne's comments addressed}}
\label{ch:resource}

\section{Hardware Resources}
Chapter~\ref{ch:est} describes the projected compute and storage needs through 2040, these resources are a shared collaboration responsibility and are requested and monitored via the \dword{ccb}, described in section \ref{sec:ccb}. Chapter~\ref{ch:netw} describes networking. 



\section{%Personnel Needs
Software Development and Operations Resources} %anne: seems to be the more current term in use
...
Although the current FTE %are 
number is close to %the number 
the identified need, there is some mismatch in skill sets. In particular, we anticipate needing several person-years of effort on framework development, after \dword{pd2} but well before \dword{fd} and \dword{nd} start, which is not yet available in the current mix of personnel.
