...
yielding of order 15 million pulses per year.  Because neutrinos interact  extremely rarely, we expect to detect of order 7,000 neutrino interactions/year in each of the four planned 10\,kt detector modules%Anne located at the \dword{fd} site in South Dakota. 
...
is available in References~\cite{DUNE:2020lwj, Abi:2020evt, Abi:2020oxb, Abi:2020loh}.  A  design report for a second detector, using \dword{vd} technology and planned to come online soon after the first \dword{hd} module is being finalized and is included in the computing planning reported here. 
...
The first two far detector modules should go live late in this decade with commissioning of the \dword{daq} %data acquisition 
systems for the first far detector modules expected to start in 2027. % anne-28.  We assume the DAQ starts firing up a year early. 
...
full-sized far detector components in 700\,tonne-capacity cryostats %anne detectors 
...
We have tested the full data taking chain from detector construction to full offline reconstruction and analysis of data, and the results have yielded considerable insight into the computing challenges for the full \dword{dune} experiment.
...
\subsubsection{Supernova candidates \hideme{Schellma -updated early march }}\label{sec:supernova}
...
\subsubsection{Other phenomena -- Solar Neutrinos and Beyond-the-Standard-Model Processes} %\hideme{???-needed}}
...
The DUNE  collaboration is proposing a suite of near detectors optimized for these two goals. The proposed detectors are described in more detail in the Near Detector Conceptual Design Report \cite{DUNE:2021tad}.
 ...
 \dword{lartpc}, \dword{ndlar}, is  the most upstream of the three subdetectors shown in Figure~\ref{nd}, where the beam propagates  from right to left. Immediately downstream of \dword{ndlar} is a detector for characterizing muons exiting the \dword{ndlar}. This will initially be a magnetized steel range stack detector (TMS). A gaseous liquid argon detector, \dword{ndgar}, which serves %anne \dword{ndlar} 
