...
The complete set of use cases that was identified for \dword{dune} has been enumerated, annotated and reviewed in detailed in \cite{bib:docdb21934} as well as the \dword{hsf} report of section~\ref{sec:req:proc}.  As a result we present and discuss here the major requirements form those reviews that impact the computing R\&D that DUNE will need to address and the use cases which have driven the collaboration to place them upon the framework develop plans.  
